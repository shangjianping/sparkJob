...
object statistics {
  ...
  def main(args:Array[String]){
    ...
    if (admin.isTableAvailable(args(0))) {
      ...
      val nameRDD = filterRDD.map(result=>(new String(result.getValue("meta".getBytes,"commodityName".getBytes)),1))
      val reduce = nameRDD.reduceByKey(_+_).map(result=>(result._2,result._1)).sortByKey()
